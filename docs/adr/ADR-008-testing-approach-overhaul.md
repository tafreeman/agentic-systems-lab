...
The `tafreeman/agentic-systems-lab` monorepo targets 80%+ test coverage (per `.claude/rules/common/testing.md`) and mandates TDD workflows. A comprehensive audit of all test files revealed two systemic problems operating in opposite directions: **test bloat** (redundant tests inflating counts without adding defect detection) and **coverage voids** (critical modules with zero or near-zero coverage).
