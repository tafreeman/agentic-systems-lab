# Integration Architecture — `tafreeman/agentic-systems-lab`
...
`agentic-workflows-v2` imports from `agentic-systems-lab-tools` in three places.
...
`agentic-v2-eval` uses lazy imports for all `agentic-systems-lab-tools` dependencies to keep the eval package installable in environments where the full tools package is not present.
...
The LLM evaluator (`evaluators/llm.py`) calls `get_llm_client()` at evaluation time. If `agentic-systems-lab-tools` is not installed, this raises `ImportError` with a clear message directing the user to install the `[llm]` extra.
