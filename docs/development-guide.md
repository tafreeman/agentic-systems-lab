...
This guide covers everything needed to set up a local development environment, run the full stack, test changes, use the CLI, and follow the project's coding standards for the `agentic-systems-lab` monorepo.
...
This installs all three Python packages (`agentic-workflows-v2`, `agentic-v2-eval`, and `agentic-systems-lab-tools`) into a shared virtual environment, respecting the version pins in `uv.lock`.
