...
def is_tracing_enabled() -> bool:
    ...
    raw = os.environ.get("AGENTIC_TRACING")
    if raw is not None:
...
def is_sensitive_capture_enabled() -> bool:
    ...
    raw = os.environ.get("AGENTIC_TRACE_SENSITIVE")
    if raw is not None:
        return raw.strip().lower() in {"1", "true", "yes", "on"}
...
def get_otlp_endpoint() -> str:
    ...
    if raw is not None:
        value = raw.strip()
        return value or DEFAULT_OTLP_ENDPOINT
    return get_settings().otel_exporter_otlp_endpoint


def get_otlp_protocol() -> str:
    """Get the OTLP protocol (grpc or http/protobuf)."""
    raw = os.environ.get("OTEL_EXPORTER_OTLP_PROTOCOL")
    if raw is not None:
        value = raw.strip().lower()
        return value or DEFAULT_OTLP_PROTOCOL
